...
PRETTIER_API_URL = f'{os.getenv('APP_URL')}/api/prettier'
...
async def calculate_prettier_sha256(filename: str) -> str:
    ...
    try:
        ...
        original_text = content.decode("utf-8")  # Convert bytes to string

        # Send text to Prettier API for formatting
        async with httpx.AsyncClient() as client:
            response = await client.post(PRETTIER_API_URL, json={"code": original_text})

        if response.status_code != 200:
            return {"error": "Prettier formatting failed", "details": response.text}

        formatted_code = response.json().get("formattedCode", original_text)

        # Compute SHA256 hash of the formatted content
